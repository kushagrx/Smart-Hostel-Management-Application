...
interface AdminPortalProps {
    ...
    isSearching: boolean;
    searchResults: any[];
    handleSearchResultPress: (result: any) => void;
    refreshing: boolean;
    onRefresh: () => void;
    recentComplaints: any[];
    pendingLeaves: any[];
    activePagingIndex: number;
    setActivePagingIndex: (index: number) => void;
    handleNavPress: (id: string) => void;
    handleComplaintAction: (id: string, status: any) => void;
    handleLeaveAction: (id: string, status: any) => void;
...
export default function AdminPortal(props: AdminPortalProps) {
    const { colors, isDark, user, insets, router } = { ...props, insets: useSafeAreaInsets(), router: useRouter() };
            ...
            <AnimatedGradientHeader style={[styles.headerBar, { paddingTop: insets.top + 16 }]}>
                <View style={styles.headerContentInner}>
                    ...
                    <View style={styles.headerTextContainer}>
                        <AppText style={styles.headerBarTitle}>Command Center</AppText>
                ...
                <View style={[styles.section, { marginBottom: 16, marginTop: 0, marginHorizontal: -24 }]}>
                    ...
                    <View style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 28, marginTop: 22, marginBottom: 18, gap: 8 }}>
                        {(['students', 'team'] as const).map(tab => (
                            <TouchableOpacity
                                key={tab}
                                onPress={() => { setActiveTab(tab); props.setActivePagingIndex(0); }}
                                style={{
                        ...
                        <>
                            <ScrollView
                                ...
                                onScroll={(e) => {
                                    ...
                                    if (index !== props.activePagingIndex) props.setActivePagingIndex(index);
                                }}
                            ...
                            <View style={styles.paginationRow}>
                                {[0, 1, 2].map((idx) => (
                                    <View key={idx} style={[styles.dot, { width: props.activePagingIndex === idx ? 12 : 6, backgroundColor: props.activePagingIndex === idx ? colors.primary : (isDark ? 'rgba(255,255,255,0.2)' : 'rgba(0,0,0,0.1)'), opacity: props.activePagingIndex === idx ? 1 : 0.5 }]} />
                                ))}
                            </View>
                        </>
                    ) : (
                                    ...
                                    <TouchableOpacity onPress={() => router.push(`/admin/complaints?openId=${c.id}`)} activeOpacity={0.7}>
                                        <View style={styles.cardHeader}>
                                            <View style={styles.studentInfoRow}>
                                                ...
                                                <View>
                                                    ...
                                                    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                                                        ...
                                                        {c.priority && (
                                                            <View style={[styles.statusBadge, { backgroundColor: c.priority === 'high' || c.priority === 'emergency' ? '#FEF2F2' : (c.priority === 'medium' ? '#FFF7ED' : isDark ? 'rgba(56, 189, 248, 0.1)' : '#E0F2FE'), paddingHorizontal: 6, paddingVertical: 2 }]}>
                                                                <AppText style={[styles.statusText, { fontSize: 10, color: c.priority === 'high' || c.priority === 'emergency' ? '#EF4444' : (c.priority === 'medium' ? '#F97316' : isDark ? '#38BDF8' : '#0284C7') }]}>Priority: {c.priority.toUpperCase()}</AppText>
                                                            </View>
                                                        )}
                                                    </View>
                                                </View>
                                            ...
                                            <View style={{ alignItems: 'flex-end', gap: 4 }}>
                                                <View style={[styles.statusBadge, { backgroundColor: c.status === 'resolved' ? '#DCFCE7' : c.status === 'inProgress' ? '#DBEAFE' : c.status === 'open' ? '#FEF9C3' : (isDark ? 'rgba(255,255,255,0.1)' : '#F1F5F9') }]}>
                                                    <AppText style={[styles.statusText, { color: c.status === 'resolved' ? '#166534' : c.status === 'inProgress' ? '#1E40AF' : c.status === 'open' ? '#854D0E' : (isDark ? '#E2E8F0' : '#475569') }]}>{c.status?.toUpperCase() || 'OPEN'}</AppText>
                                                </View>
                                                <AppText style={{ fontSize: 11, color: colors.textSecondary, fontWeight: '700' }}>{c.createdAt ? formatUniversalTime(c.createdAt, { hour: 'numeric', minute: '2-digit', hour12: true }) : ''}</AppText>
                                            </View>
                                        </View>
                                        <View style={styles.cardContentDetailed}>
                                            <AppText style={styles.cardTitle} numberOfLines={2}>{c.status === 'resolved' ? <AppText style={{ textDecorationLine: 'line-through' }}>{c.title}</AppText> : c.title}</AppText>
                                        ...
                                        <View style={{ flexDirection: 'row', gap: 8, marginTop: 12 }}>
                                            {c.status === 'open' && (
                                                <TouchableOpacity style={[styles.actionBtn, { backgroundColor: isDark ? 'rgba(59, 130, 246, 0.15)' : '#EFF6FF' }]} onPress={() => props.handleComplaintAction(c.id, 'inProgress')}>
                                                    <AppText style={{ color: '#3B82F6', fontWeight: '700', fontSize: 13 }}>{props.loadingActions[`complaint-${c.id}`] === 'inProgress' ? 'Marking...' : 'Mark In-Progress'}</AppText>
                                                </TouchableOpacity>
                                            )}
                                            <TouchableOpacity style={[styles.actionBtn, { backgroundColor: isDark ? 'rgba(34, 197, 94, 0.15)' : '#F0FDF4' }]} onPress={() => props.handleComplaintAction(c.id, 'resolved')}>
                                                <AppText style={{ color: '#22C55E', fontWeight: '700', fontSize: 13 }}>{props.loadingActions[`complaint-${c.id}`] === 'resolved' ? 'Resolving...' : 'Resolve Issue'}</AppText>
                                            </TouchableOpacity>
                                        </View>
                                    )}
                                </View>
                                    ...
                                    <TouchableOpacity onPress={() => router.push(`/admin/leaveRequests?openId=${l.id}`)} activeOpacity={0.7}>
                                        <View style={styles.cardHeader}>
                                            ...
                                            <View style={{ alignItems: 'flex-end', gap: 4 }}>
                                                ...
                                                <AppText style={{ fontSize: 11, color: colors.textSecondary, fontWeight: '700' }}>{l.createdAt ? formatUniversalTime(l.createdAt, { hour: 'numeric', minute: '2-digit', hour12: true }) : ''}</AppText>
                                            </View>
                                        </View>
                                        <View style={styles.cardContentDetailed}>
                                    ...
                                    {l.status === 'pending' && (
                                        <View style={{ flexDirection: 'row', gap: 8, marginTop: 8 }}>
                                            <TouchableOpacity style={[styles.actionBtn, { backgroundColor: isDark ? 'rgba(239, 68, 68, 0.12)' : '#FEF2F2' }]} onPress={() => props.handleLeaveAction(l.id, 'rejected')}>
                                                <AppText style={{ color: '#EF4444', fontWeight: '700', fontSize: 13 }}>{props.loadingActions[`leave-${l.id}`] === 'rejected' ? 'Rejecting...' : 'Reject'}</AppText>
                                            </TouchableOpacity>
                                            <TouchableOpacity style={[styles.actionBtn, { backgroundColor: isDark ? 'rgba(34, 197, 94, 0.15)' : '#F0FDF4' }]} onPress={() => props.handleLeaveAction(l.id, 'approved')}>
                                                <AppText style={{ color: '#22C55E', fontWeight: '700', fontSize: 13 }}>{props.loadingActions[`leave-${l.id}`] === 'approved' ? 'Approving...' : 'Approve'}</AppText>
                                            </TouchableOpacity>
                                        </View>
                                    )}
                                </View>
...
const createStyles = (colors: any, isDark: boolean) => StyleSheet.create({
    ...
    studentAvatar: { width: 38, height: 38, borderRadius: 19, backgroundColor: isDark ? '#334155' : '#F1F5F9', marginRight: 10 },
